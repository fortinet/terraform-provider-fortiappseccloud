...
func NewOpenapiValidationCreateClient(c *CloudWafClient, d interface{}, uploadFiles []UploadFiles) (*OpenapiValidationCreateClient, error) {
	...
	request, err := NewRequestWithFiles(c, "PUT", "waf/apps/"+data.EPId+"/api_protection", nil, bytes, uploadFiles)
	output("URL >>: " + "waf/apps/" + data.EPId + "/api_protection")
	if err != nil {
		log.Print(err)
		return nil, err
	}
...
func NewOpenapiValidationQueryClient(c *CloudWafClient, d interface{}) *OpenapiValidationQueryClient {
	...
	request := NewRequest(c, "GET", "waf/apps/"+data.EPId+"/api_protection", nil, nil)
